...
from yolo_v1 import get_tagged_img, decode_predictions
...
class YoloV1Generator3(keras.utils.Sequence):
...
def test(img, label_path, names_path):
    width = img.shape[1]
    height = img.shape[0]
...
if __name__ == "__main__":
    ...
    generator_1 = YoloV1Generator2(data_dir, input_shape, batch_size, num_classes=3, num_boxes=2, augment=True, shuffle=False)
    generator_2 = YoloV1Generator2(data_dir, input_shape, batch_size, num_classes=3, num_boxes=2, augment=False, shuffle=False)
    generator_3 = YoloV1Generator3(data_dir, input_shape, batch_size, num_classes=3, num_boxes=2, transforms=transforms)
    img_path_array = generator_3.img_path_array

    cv2.namedWindow('Aug')
    cv2.resizeWindow('Aug', 448, 448)

    for idx in range(generator_3.__len__()):
        batch_x, batch_y = generator_3.__getitem__(idx)
        img_path = img_path_array[idx]
        label_path = img_path.replace('.jpg', '.txt')
        
        batch_x = cv2.cvtColor(batch_x[0], cv2.COLOR_RGB2BGR)
        tagged_img = get_tagged_img(batch_x, decode_predictions(batch_y, 3), names_path)

        cv2.imshow('Aug', tagged_img)
        key = cv2.waitKey(0)
        if key == 27:
